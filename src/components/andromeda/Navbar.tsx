    ...
    <>
      <header
        className={[
          ...
          scrolled || menuOpen
            ? 'bg-bg/97 shadow-sm backdrop-blur-[12px] border-b border-charcoal/[.06]'
            : '',
        ].join(' ')}
      >
        {/* Nav bar */}
        <div
          className={`flex items-center justify-between px-[4vw] transition-all duration-500 ${
            scrolled || menuOpen ? 'py-3' : 'py-5'
          } ${!scrolled && !menuOpen ? 'bg-gradient-to-b from-black/40 to-transparent' : ''}`}
        >
          <a href="#hero" className="flex flex-col gap-0.5 no-underline" aria-label="Andrômeda Home">
            <span className={`font-serif text-[1.5rem] tracking-[.02em] transition-colors duration-300 ${scrolled || menuOpen ? 'text-charcoal' : 'text-white'}`}>Andrômeda Home</span>
          </a>
      ...
      {/* Mobile menu — fixed fora do header para z-index independente */}
      {menuOpen && (
        <div
          className="fixed inset-0 bg-bg/[.98] z-[101] flex flex-col items-center justify-center gap-8"
          role="dialog"
        ...
        >
          ...
          <button
            onClick={() => { setMenuOpen(false); onOpenVisitModal() }}
