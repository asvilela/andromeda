...
import { FLOOR_PLANS, type FloorPlan } from '@/lib/constants'
...
function FloorPlanPanel({ tipo, isActive, onOpenLightbox }: FloorPlanPanelProps) {
  ...
  return (
    // Kept in the DOM (not conditionally mounted) and merely hidden via CSS when
    // inactive, so every floor plan's specs are crawlable regardless of which
    // tab a visitor last clicked — Google treats display:none tab content as
    // fully indexable, unlike content that's never rendered at all.
    <div
      className={`grid grid-cols-1 lg:grid-cols-2 gap-10 lg:gap-16 items-center transition-opacity duration-200 ${fading ? 'opacity-0' : 'opacity-100'} ${isActive ? '' : 'hidden'}`}
      role="tabpanel"
    ...
    >
      <div className="flex flex-col gap-4">
        ...
        <div
          ...
          onClick={() => onOpenLightbox(activeVariant)}
          role="button"
          tabIndex={0}
          aria-label="Ampliar planta"
          onKeyDown={(e) => e.key === 'Enter' && onOpenLightbox(activeVariant)}
        >
...
export default function Tipologias() {
  ...
  const openLightbox = useCallback((variantId: string) => {
    const idx = ALL_VARIANTS.findIndex((v) => v.id === variantId)
  ...
  return (
    <section id="tipologias" className="bg-bg py-20 sm:py-28 px-[8vw]" aria-labelledby="tipologias-title">
      <div className="max-w-[1200px] mx-auto">
        ...
        {FLOOR_PLANS.map((t) => (
          <FloorPlanPanel key={t.id} tipo={t} isActive={active === t.id} onOpenLightbox={openLightbox} />
        ))}
      </div>
